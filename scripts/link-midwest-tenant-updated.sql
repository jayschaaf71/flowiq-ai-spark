-- Link Midwest Admin to Midwest Dental Sleep Tenant (Updated)
...
SELECT id, email, created_at FROM auth.users WHERE email = 'midwest.admin@flowiq.test';

SELECT '=== FINDING MIDWEST PROFILE ===' as status;
SELECT id, first_name, last_name, contact_email, role FROM public.profiles WHERE contact_email = 'midwest.admin@flowiq.test';
