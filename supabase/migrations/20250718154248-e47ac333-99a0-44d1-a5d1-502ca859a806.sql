-- Create comprehensive cost tracking and analytics tables (fixed version)
...
-- Margin Analysis Table (fixed - no cascading generated columns)
CREATE TABLE public.tenant_margins (
  ...
  profitability_status TEXT DEFAULT 'break_even' CHECK (profitability_status IN ('highly_profitable', 'profitable', 'break_even', 'unprofitable')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- Create function to update profitability status
CREATE OR REPLACE FUNCTION public.update_profitability_status()
RETURNS TRIGGER AS $$
BEGIN
  NEW.profitability_status = CASE 
    WHEN NEW.margin_percentage >= 20 THEN 'highly_profitable'
    WHEN NEW.margin_percentage >= 10 THEN 'profitable' 
    WHEN NEW.margin_percentage >= 0 THEN 'break_even'
    ELSE 'unprofitable'
  END;
  RETURN NEW;
END;
...
-- Create trigger to automatically update profitability status
CREATE TRIGGER update_margin_profitability_status
  BEFORE INSERT OR UPDATE ON public.tenant_margins
  FOR EACH ROW EXECUTE FUNCTION public.update_profitability_status();